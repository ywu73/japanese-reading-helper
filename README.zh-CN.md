...
YomiRuby 是一个隐私边界明确的 Tampermonkey 日语网页阅读辅助工具。它可以通过独立选择的本地、Google 或 Bing 模式，在含汉字词上方显示罗马音；也可以通过另行选择的联网服务，在匹配到的片假名词组上方显示尽力而为的英文注音。

**0.5.0 当前只是经过本地验证的发布候选；桌面 Chrome + Tampermonkey 安装、扩展后台抓包、更新、真实网站和发布门槛尚未完成。**
...
- 本地模式在页面内运行固定版本 `kuromoji@0.1.2`，并默认使用带长音符号的 modified Hepburn，例如 `kyō`、`Tōkyō`。0.5.0 过渡版仍预加载全部 12 个词典资源。
- Google/Bing 模式使用本地 `Intl.Segmenter` 划分词边界，每次只发送一个完整、去重且含汉字的词，不发送完整句子、周围上下文、页面标题、页面 URL、origin 或浏览历史。
...
在 0.5.0 的浏览器与发布门槛正式记录为完成之前，不应把该地址视为已经发布的稳定版本。首个公开版本的兼容目标只包括 **桌面 Google Chrome + Tampermonkey**；其他浏览器和用户脚本管理器尚未验证，不提供兼容承诺。
...
| Google Translate | 只有当前精确 origin 已开启对应联网功能，且选择 Google 时 | 片假名功能通过 `q` 发送有界的匹配词组；汉字模式每次通过 `q` 发送一个本地分出的完整含汉字词，并请求独立的 `dt=rm` 源读音。 |
| Bing Translator | 只有当前精确 origin 已开启对应联网功能，且选择 Bing 时 | 先匿名 GET 临时配置而不执行页面脚本；片假名功能以 `ja -> en` 发送有界、换行连接的精确词组批次并严格逐行验收，汉字模式以 `ja -> ja` 每次发送一个本地分出的词，并只接受独立的 `inputTransliteration`。 |
...
YomiRuby 自有代码与贡献采用 [MIT License](LICENSE)。第三方许可证与来源记录见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。0.5.0 不要求 CLA 或 DCO。
