...
**日语阅读助手 — 汉字罗马音与片假名英译** 是一个隐私边界明确的 Tampermonkey 日语网页阅读辅助工具。它可以通过独立选择的本地、Google 或 Bing 模式，在含汉字词上方显示罗马音；也可以通过另行选择的联网服务，在匹配到的片假名词组上方显示尽力而为的英文翻译。

**0.6.2 统一使用新的中英文名称。桌面 Chrome + Tampermonkey 完整安装、扩展后台抓包、自动更新和真实网站兼容性仍未完成验证。**

项目原名 **YomiRuby**。本次统一使用「日语阅读助手 / Japanese Reading Helper」；为延续已有安装，设置键、命名空间和安装文件名仍保留 `yomi-ruby`。详见[改名与升级说明](docs/rename-0.6.2.md)。
...
- 本地模式在页面内运行固定版本 `kuromoji@0.1.2`，并默认使用带长音符号的 modified Hepburn，例如 `kyō`、`Tōkyō`。0.6.2 仍预加载全部 12 个词典资源。
...
- 片假名的每批结果通过校验后即显示；后续批次失败时保留前面已经成功的标注。
...
日语阅读助手匹配普通 HTTP/HTTPS 页面，并使用 `@noframes`。全站匹配是为了让用户能在任意网站选择开启日语阅读助手，并不表示功能会在所有网站自动运行：**每个未配置的精确 origin 上，两项功能都默认关闭**。

## 控制与语言

日语阅读助手按“汉字、汉字模式、片假名、片假名翻译服务、语言”的固定顺序注册五个 Tampermonkey 菜单。正常的开启、关闭、启动、模式切换、服务切换和语言切换不会出现授权确认框、加载横幅或成功横幅。只有设置写入失败、安全启动失败等可处理错误才会显示临时非模态提示。
...
界面支持英文和简体中文。仅在第一次运行且没有已存语言时，日语阅读助手会读取浏览器首选语言：以 `zh` 开头时映射为简体中文，其他情况映射为英文，然后全局保存。手动切换语言会永久覆盖首次检测结果。语言切换不会改变功能开关、重新扫描文字、加载 Kuromoji 或发送请求；它只会立即重排汉字模式菜单的下一步操作，而不改变当前模式。没有有效片假名服务设置时，日语阅读助手会根据已解析的界面语言一次性选择默认值（`zh` -> Bing，其他 -> Google）并保存；之后切换语言不会覆盖服务设置。
...
日语阅读助手不包含项目自有分析、遥测、崩溃上报、远程日志、跟踪标识、安装回调或跨服务静默降级。仅允许以下翻译路由：
...
- 跳过脚本、样式、表单、编辑区、代码、隐藏内容、现有 Ruby、SVG/MathML、媒体和日语阅读助手自有界面。
...
- DOM Coordinator 是唯一 Ruby 所有者，使用约 500ms 的事件驱动合并窗口与协作式分块，不使用永久 interval。独立的视口调度器优先处理父元素位于屏幕内及周围 300px 的文字；前台请求完成后，以每组最多 32 个记录逐步处理屏外内容，即使不滚动也会补齐。没有视口观察能力时仍按普通顺序扫描。后台标签不启动新任务，回到前台后补扫当前仍连接的 DOM。
- 关闭后停止队列和观察器；过期或已取消的异步结果不能重新写入页面。

## Katakana Terminator 致谢

日语阅读助手的可选联网片假名英文模块基于 Arnie97 与 Katakana Terminator Contributors 开发的 Katakana Terminator。该模块改编了 Katakana Terminator 的片假名匹配模式、精确候选去重思想和 Google Translate 请求方案。日语阅读助手的汉字罗马音模式、Kuromoji 校验加载、Google/Bing 源罗马化客户端、两个独立深运行时、前台整页 DOM 协调、请求取消、响应验收、可逆生命周期和双语控制属于独立实现。Katakana Terminator 使用 MIT License。
...
`src/` 是源代码真相；`dist/yomi-ruby.user.js` 只能通过 `scripts/build.mjs` 生成，禁止手工修改。构建会嵌入规范的日语阅读助手和第三方许可证/NOTICE，并审计版本、元数据、存储范围、请求路径、禁止能力、固定资源与法律材料。
...
日语阅读助手自有代码与贡献采用 [MIT License](LICENSE)。第三方许可证与来源记录见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。0.6.2 不要求 CLA 或 DCO。
